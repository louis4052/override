...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private lazy var loggerManager = DiagnosticLogger()

    private lazy var logger = loggerManager.logger(forCategory: "AppDelegate")

    private lazy var analyticsManager = AnalyticsManager(loggerManager: loggerManager)

    var window: UIWindow?

    private(set) lazy var deviceManager = DeviceDataManager(loggerManager: loggerManager, analyticsManager: analyticsManager)
    ...
    func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([Any]?) -> Void) -> Bool {
        ...
        if #available(iOS 12.0, *) {
            if userActivity.activityType == NewCarbEntryIntent.className {
                logger.default("Restoring \(userActivity.activityType) intent")
        ...
        case NSUserActivity.newCarbEntryActivityType,
            ...
            logger.default("Restoring \(userActivity.activityType) activity")
